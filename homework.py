...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        """Получить количество затраченных калорий."""
        return ((self.running_calorie_multiplier_1 * self.get_mean_speed()
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        time_min: int = 60
        calories_calc_walk = ((calories_3 * self.weight + 
        (super().get_mean_speed() ** 2 // self.height) *
        calories_4 * self.weight) * (self.duration * time_min))
        return calories_calc_walk
...
class Swimming(Training):
    ...
    def __init__(self, action: int, duration: float, weight: float, length_pool: int, 
                 count_pool: int) -> None:
        super().__init__(action, duration, weight)
    ...
    def get_spent_calories(self) -> float:
        ...
        calories_6: int = 2
        calories_calc_swim = ((Swimming.get_mean_speed(self) + calories_5) * 
        (calories_6) * (self.weight))
        return calories_calc_swim    
